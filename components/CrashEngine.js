...
export default function CrashEngine() {
  ...
  const placeBet = async () => {
    ...
    if (data.success) {
      ...
      setMessage({ type: 'success', text: `Bet Pi ${betAmount} placed! Cash out before it crashes!` })
    } else {
  ...
  const cashOut = async () => {
    ...
    if (data.success) {
      setActiveBet(null)
      setWallet(w => ({ ...w, balance: data.new_balance }))
      setMessage({ type: 'success', text: `Cashed out! +Pi ${data.payout.toFixed(2)} at ${displayMultiplier}x` })
    }
  }
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '16px' }}>
      ...
      {wallet && (
        <div style={{ background: '#1a1a1a', padding: '8px 24px', borderRadius: '24px', border: '1px solid #333', fontSize: '14px' }}>
          💰 Wallet: <strong style={{ color: 'var(--accent)' }}>Pi {parseFloat(wallet.balance).toFixed(2)}</strong>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{ marginTop: '24px', display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          {!activeBet ? (
            ...
            >
              CASH OUT<br />
              <small>Pi {(activeBet.amount * displayMultiplier).toFixed(2)}</small>
            </button>
          )}
        </div>
