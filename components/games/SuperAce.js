    ...
    if (winCells.length > 0) {
      ...
      if (finalWin > 0) {
        ...
        if (finalWin >= betAmount * 10) {
          ...
          setMessage({ type: 'success', text: `Cascade win! +Pi ${finalWin.toFixed(2)} (${activeMult}x Mult)` })
        }
      }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        >
          {spinning ? 'SPINNING...' : isFreeSpins ? `FREE SPIN MODE (${freeSpinsLeft})` : `SPIN - Pi ${betAmount}`}
        </button>
      </div>
