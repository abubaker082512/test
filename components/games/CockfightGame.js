...
export default function CockfightGame({ user, wallet, fetchWallet }) {
  ...
  const startFight = async () => {
    ...
    setStage('fighting')
    addLog(`📢 Fight Started! Stake Pi ${betAmount} placed on ${betChoice}.`)
  ...
  const resolveCombat = async (finalMeronHP, finalWalaHP) => {
    ...
    if (isWin) {
      ...
      if (payData.success) {
        setMessage({ type: 'success', text: `🎉 WINNER! ${winner} won the fight! +Pi ${(betAmount * multiplier).toFixed(2)}` })
      }
    } else {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        >
          {stage === 'fighting' ? 'COMBAT UNDERWAY...' : `MATCH FIGHT — Pi ${betAmount}`}
        </button>
      </div>
