...
export default function FishingJoy({ user, wallet, fetchWallet }) {
  ...
  const [betAmount, setBetAmount] = useState(10) // Pi 10 to buy 20 ammo shots
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      coinsRef.current = coinsRef.current.filter(coin => {
        ...
        ctx.textAlign = 'center'
        ctx.fillText(`+Pi ${coin.val}`, coin.x, coin.y - 12)
  ...
  const collectWinnings = async () => {
    ...
    if (data.success) {
      setMessage({ type: 'success', text: `🎉 Coins gathered! +Pi ${sessionWin.toFixed(2)} added to wallet.` })
      setSessionWin(0)
      setAmmo(0)
    }
    setProcessing(false)
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '20px', color: 'var(--accent)', fontWeight: 'bold', fontSize: '14px', marginBottom: '8px' }}>
        ...
        <span>💵 Coins: Pi {sessionWin.toFixed(2)}</span>
      </div>
      ...
      <div style={{
        ...
        {ammo === 0 ? (
          <button 
            ...
            disabled={processing}
          >
            {processing ? 'CHARGING...' : `BUY 20 AMMO — Pi ${betAmount}`}
          </button>
        ) : (
          <button 
            className="btn" 
          ...
          >
            {processing ? 'COLLECTING...' : `📥 Cash Out Pi ${sessionWin.toFixed(2)}`}
          </button>
        )}
      </div>
    </div>
