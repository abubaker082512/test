...
export default function BlackjackGame({ user, wallet, fetchWallet }) {
  ...
  const finishGame = async (pHand, dHand, result) => {
    ...
    if (result === 'blackjack') {
      multiplier = 2.5
      outcomeMsg = `🃏 Blackjack! Double Cash! +Pi ${(betAmount * 2.5).toFixed(2)}`
      isWin = true
    } else if (result === 'win' || result === 'dealer-bust') {
      multiplier = 2.0
      outcomeMsg = result === 'dealer-bust' ? `Dealer Busted! You Won! +Pi ${(betAmount * 2).toFixed(2)}` : `You Won! +Pi ${(betAmount * 2).toFixed(2)}`
      isWin = true
    } else if (result === 'push') {
      multiplier = 1.0
      ...
      <div className="game-glass-panel" style={{ maxWidth: '400px', border: '1px solid rgba(0, 230, 118, 0.25)', background: 'rgba(5, 17, 10, 0.7) !important' }}>
        ...
        {gameStage === 'idle' || gameStage === 'ended' ? (
          <div style={{ 
            ...
            >
              {loading ? 'DEALING CARDS...' : `PLACE BET : Pi ${betAmount}`}
            </button>
          </div>
        ) : (
          <div style={{ 
