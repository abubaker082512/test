  ...
  const autoCashOut = async (goals) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({
          type: 'success',
          text: `🏆 STREAK COMPLETED! 5/5 Goals Scored. Cashed Out Pi ${(betAmount * mult).toFixed(2)}!`
        })
      }
    } catch (e) {
  ...
  const cashOutRound = async () => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({
          type: 'success',
          text: `💰 Cashed Out Safely! Won Pi ${(betAmount * mult).toFixed(2)} (${mult}x)`
        })
      }
    } catch (e) {
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {!activeRound ? (
            ...
            <>
              ...
              >
                CASH OUT [Pi {(betAmount * (goalsCount > 0 ? MULTIPLIERS[goalsCount - 1] : 0)).toFixed(2)}]
              </button>
            </>
          )}
        </div>
