    ...
    setTimeout(async () => {
      ...
      selectedChips.forEach(chip => {
        ...
        if (isWin) {
          ...
          details.push(`Pi ${(chip.amount * payoutMult).toFixed(2)} on ${chip.value.toString().toUpperCase()}`)
        }
      })
      ...
      if (totalPayout > 0) {
        try {
          ...
          if (payData.success) {
            setPayoutMsg({
              type: 'success',
              text: `🎉 WINNER! Landed ${targetObj.color.toUpperCase()} ${targetObj.n}! Payout: +Pi ${totalPayout.toFixed(2)} (${details.join(', ')})`
            })
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        >
          {spinning ? 'SPINNING...' : `SPIN WHEEL — Pi ${totalCurrentBet}`}
        </button>
      </div>
    </div>
  )
