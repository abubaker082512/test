...
export default function CrashGame({ user, wallet, fetchWallet }) {
  ...
  const triggerCashOut = async () => {
    ...
    if (payData.success) {
      setMessage({ type: 'success', text: `💰 Cashed Out! +Pi ${(betAmount * currentMult).toFixed(2)} (${currentMult}x)` })
    }
    fetchWallet()
  }
        ...
        <div style={{
          ...
          {stage !== 'running' ? (
            <>
              ...
              >
                {processing ? 'CHARGING ENGINES...' : `BET Pi ${betAmount} & LAUNCH`}
              </button>
            </>
          ) : (
            <button 
            ...
            >
              CASH OUT : Pi {(betAmount * multiplier).toFixed(2)}
            </button>
          )}
        </div>
              ...
              {players.map((p, idx) => (
                ...
                >
                  ...
                  <span style={{ fontWeight: 'bold', color: p.cashedOut ? '#00e676' : '#ffea00' }}>
                    {p.cashedOut ? `${p.cashOutMult.toFixed(2)}x` : `Pi ${p.bet}`}
                  </span>
                </div>
              ))}
            </div>
